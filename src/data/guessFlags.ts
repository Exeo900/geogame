...
export interface FlagQuestion extends Country {
  difficulty: Difficulty
}

// Länder taggade med svårighetsgrad baserat på hur välkänd flaggan är
const easyCodes: string[] = [
  // Välkända europeiska flaggor
...
export const flagQuestions: FlagQuestion[] = countries
  .filter((c) => difficultyMap.has(c.code))
  .map((c) => ({ ...c, difficulty: difficultyMap.get(c.code)! }))

export function getByDifficulty(difficulty: Difficulty): FlagQuestion[] {
  return flagQuestions.filter((q) => q.difficulty === difficulty)
}
